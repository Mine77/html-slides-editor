...
test("image floating toolbar hides text and color tools and supports crop handles", async ({
  ...
  const croppedImageRect = await getSlideElementRect(editableImage);
  await expect(page.getByTestId("selection-overlay")).toBeVisible();
  await expect(page.getByTestId("block-resize-handle-top-left")).toBeVisible();
  const selectionOverlayAfterExit = await page.getByTestId("selection-overlay").boundingBox();
  expect(selectionOverlayAfterExit).not.toBeNull();
  if (!selectionOverlayAfterExit) {
    throw new Error("Expected selection overlay to have bounds after exiting crop mode.");
  }
  expect(selectionOverlayAfterExit.width).toBeLessThan(croppedImageRect.width);
  expect(selectionOverlayAfterExit.height).toBeLessThan(croppedImageRect.height);
