...
### E2E 测试维护
以上需求涉及 UI 交互变更，会影响现有 e2e 测试（`e2e/tests/`）。实现时需：
- 更新受影响的测试断言（如删除侧边栏后，sidebar 相关测试需要移除或改写）
- 为新功能补充 e2e 测试（context menu、group/ungroup、resize scaling 等）
- 确保 `pnpm test:e2e` 在每个需求完成后通过

### ADR 文档
每个需求实现前/后，补充对应的架构决策记录（`docs/adr/`），记录：
- 决策背景和动机
- 方案选型和权衡
- 对现有架构的影响
